...
QqQ="QqQe308";banana="3.8642180e38642180";Liu="6.666666666666666666666666e308";fufu=['cutefu~','c','u','t','e','f','u','~'];Loader="也可以试试Loader的音乐游戏树mod哦！链接：https://qq1010903229.github.io/The-Rhythm-Game-Tree/";yszqzls="元素周期增量树是不是停更了😡";yyyxs="音乐游戏树，鱼鱼永相随，游泳一小时，原原原x神";Genshin="启动！";Phigros="Phigros什么时候更新急急急";long2024="龙年马上要到了！祝大家新年龙腾虎跃，龙飞凤舞，龙批一个，事业有成，学业顺利，身体健康，财源滚滚，音游全AP，考试全满分，工作全加薪，玩树全通关，再次献上音乐游戏树全体作者（共1人）的真挚祝福！！！！！！！！！！（已经八月份了，我觉得这个彩蛋完全没有意义）";QqQe308="我是QqQe308，v我50更新音乐游戏树"}
}
	},
...
style() { return { 'background-color': hasAchievement('A',1003)?"#308308":"#ff4848"}},
     ...
     1004: {
     name: "隐藏成就4",
     done() {return inChallenge('a',11)&&inChallenge('p',11)&&inChallenge('c',11)&&inChallenge('r',11)&&inChallenge('ri',11)},
     tooltip() {
     if(hasAchievement('A',1004)) return "The first is the five challenges.<br>同时进入5个层级的第一个挑战<br>奖励：解锁某一个层级的挑战"
     else return "The first is the five challenges."
     },
...
style() { return { 'background-color': hasAchievement('A',1011)?"#308308":"#ff4848"}},
     ...
     style() { return { 'background-color': hasAchievement('A',1013)?"#308308":"#ff4848"}},
     },
    },
    tabFormat: {
   "Achievements": {
...
if(hasAchievement('A',71)) exp=exp.add(0.1)
   ...
   "main-display",
    "prestige-button",
     ["display-text",
     function() {return '你有 ' + format(player.points) + ' Notes<br>你正在获得 ' + format(n(tmp.s.resetGain).mul(tmp.a.passiveGeneration))+' 歌曲每秒'},
...
return sna
    ...
    snRandom() {
     let sn=[1,2]
     if(hasUpgrade('sp',14)) sn.push(3,4)
     if(hasUpgrade('ch',53)) sn.push(5)
     if(hasUpgrade('ch',55)) sn.push(6)
     return sn[n(Math.random()).mul(sn.length).floor().toNumber()]
    },
    snEff1() {
 ...
 if(hasMilestone('p',2)||hasMilestone('c',0))upg=true
  ...
  clickables: {
    ...
    12: {
     ...
     player.a.pttMax = player.a.pttMax.add(pttMax).min(tmp.ri.arc1.max(13))
     },
     onHold() {
     let pttMax=player.l.points.min(35).add(1).log(10).add(1).pow(2).sub(player.a.pttMax.mul(player.a.pttMax2)).div(2).max(0)
     player.a.pttMax = player.a.pttMax.add(pttMax).min(tmp.ri.arc1.max(13))
...
player.a.snRandom=tmp.a.snRandom},
...
tabFormat: {
  ...
  unlocked(){return hasUpgrade('a',15)}
   ...
   "main-display",
    ...
    ["display-text",
     function() {if(player.a.pttMax.gte(tmp.ri.arc1)) return 'PTT在'+format(tmp.ri.arc1.max(13))+'时达到硬上限！'},
  ...
  unlocked(){return hasUpgrade('l',11)||hasMilestone('p',1)}
   ...
   "main-display",
    ...
    ["display-text",
     ...
     {"color": "#ffffff", "font-size": "18px", "font-family": "Comic Sans MS"}],
     ["display-text",
     function() {if(hasUpgrade('sp',22)) return '5.源点获取指数+' + format(tmp.a.snEff5,3)},
     {"color": "#ffffff", "font-size": "18px", "font-family": "Comic Sans MS"}],
     "blank",
    ...
    ["display-text",
     ...
     {"color": "#ffffff", "font-size": "18px", "font-family": "Comic Sans MS"}],
     ["display-text",
     function() {if(hasUpgrade('sp',21)) return '2.软上限前蛇的所有效果^' + format(tmp.a.drEff2,3)},
     {"color": "#ffffff", "font-size": "18px", "font-family": "Comic Sans MS"}],
     ["display-text",
     function() {if(hasUpgrade('sp',22)) return '3.课题力量^' + format(tmp.a.drEff3,3)},
     {"color": "#ffffff", "font-size": "18px", "font-family": "Comic Sans MS"}],
     ["display-text",
     function() {if(hasUpgrade('sp',25)) return '4.Notes获取量^' + format(tmp.a.drEff4,3)},
     {"color": "#ffffff", "font-size": "18px", "font-family": "Comic Sans MS"}],
     ["display-text",
     function() {if(hasUpgrade('sp',27)) return '5.课题模式总分×' + format(tmp.a.drEff5,3)},
     {"color": "#ffffff", "font-size": "18px", "font-family": "Comic Sans MS"}],
 ...
 if(hasMilestone('c',3))upg=true
  ...
  challenges: {
   ...
   if(hasUpgrade('p',21)){return "歌曲和源点获取为原来的平方根<br>完成次数:"+challengeCompletions(this.layer,this.id)+"/10"}},
     goalDescription(){return "1e"+n(8).add(n(challengeCompletions(this.layer,this.id)).mul(4))+"源点"},
  ...
  unlocked(){return hasUpgrade('p',13)}
   ...
   "main-display",
    ...
    ["display-text",
     ...
     {"color": "#ffffff", "font-size": "9px", "font-family": "Comic Sans MS"}],
     ["display-text",
     function() {if(player.p.rksMax.gte(tmp.ri.phi1)) return 'RKS在'+format(tmp.ri.phi1)+'时达到硬上限！'},
...
addLayer("m", {
 ...
 effectDisplay() { return format(upgradeEffect(this.layer, this.id))+"×" },
    unlocked() {return hasUpgrade('m',15)}, },//如果作者下一次更新没有添加升级17并且删掉这条注释请提醒我或者超我
   },
    milestones: {
    0: {
...
addLayer("c", {
 ...
 introBox: {
    ...
    powerCal(){
     ...
     if(tmp.ch.tapEff.gte(1)) mult = mult.times(tmp.ch.tapEff)
...
addLayer("ch", {
 ...
 introBox: {
   ...
   color: "#ffe000",
    ...
    noteMax() {
     let max=n(4000)
     max=max.add(clickableEffect('e',16))
     return max
    },
    note() {
    ...
    if(buyableEffect('c',31)>1) exp = exp.times(buyableEffect('c',31))
     return mult.min(tmp.ch.noteMax)
    },
    dif() {
     let dif = player.ch.note.div(50).pow(0.8).max(1)
     return dif
    },
    pnote() {
     let pnote = player.ch.note.div(10).pow(0.8).max(0)
     if(hasUpgrade('e',24)&&!hasUpgrade('ri',44)) pnote=n(0)
     return pnote.floor()
    },
    pnoteNext() {
     let pnote = player.ch.pnote.add(1)
     if(hasUpgrade('e',24)&&!hasUpgrade('ri',44)) pnote=n(1/0)
     return pnote.max(0).root(0.8).mul(10)
    },
    tapEff() {
    let tap = n(2).pow(player.ch.tap.max(0))
    tap=tap.pow(player.ch.dif)
    tap=tap.pow(clickableEffect('e',17))
    if(tap.log10().gte(2500)) tap = n(10).pow(tap.log10().sub(2500).pow(0.5).add(2500))//sc
     return tap},
    dragEff() {
     let drag = player.ch.drag.add(1).log(10).div(10).add(1).min(1.5)
    drag=drag.pow(player.ch.dif.max(1).log(10).add(1)).min(2)
     return drag.min(1.2)},
    flickEff() {
   let flick = player.ch.flick.add(1).log(5).div(5).add(1).min(2)
    flick=flick.pow(player.ch.dif.max(1).log(10).add(1)).min(3)
     return flick.min(1.62)},
    holdEff() {
   let hold = player.ch.hold.add(1).log(100).div(25).add(1).min(1.2)
    hold = hold.pow(player.ch.dif.max(1).log(20).add(1)).min(1.44)
     return hold.min(1.04)},
    keti() {
   let rks=player.p.rks
   let dif1=player.ch.dif1
   let dif2=player.ch.dif2
   let dif3=player.ch.dif3
   let a1=n(Math.random()).add(0.1).pow(0.05)
   let a2=n(Math.random()).add(0.1).pow(0.05)
   let a3=n(Math.random()).add(0.1).pow(0.05)
    if(hasUpgrade('ch',42)){
    ...
    enp() {//课题力量
     ...
     if(clickableEffect('e',14).gt(1)) enp=enp.times(clickableEffect('e',14))
    ...
    enpEff() {//课题力量效果
     let enp=player.ch.enp
     let power=n(2)
     if(hasUpgrade('ch',54)) power=n(3)
     if(hasUpgrade('j',16)) power=power.mul(1.5)
     if(hasUpgrade('e',22)) power=power.sub(upgradeEffect('e',22))
     let eff=enp.pow(power)
     return eff.max(1)
},
    row: 3, 
   ...
   "main-display",
    "prestige-button",
     ...
     {"color": "#ffffff", "font-size": "20px", "font-family": "Comic Sans MS"}],
     ["display-text",
     function() {return '物量上限: ' + format(tmp.ch.noteMax)},
     {"color": "#ffffff", "font-size": "20px", "font-family": "Comic Sans MS"}],
     ["display-text",
...
unlocked(){return hasMilestone('ch',1)}
   ...
   "main-display",
    "prestige-button",
     "blank",
    ["display-text",
     function() { return '你有 ' + format(player.ch.pnote) + ' 个Phigros-Note(PN)<br>下一个需要 '+format(tmp.ch.pnoteNext)+' 物量<br>当前物量：'+format(player.ch.note)},
     {"color": "#ffffff", "font-size": "20px", "font-family": "Comic Sans MS"}],
     "blank",
     "blank",
    ["display-text",
     ...
     "blank",
     ["display-text",
     function() {if(hasUpgrade('ch',22)) return '你有 ' + format(player.ch.drag) + ' 个黄键<br>黄键对应效果：Note^'+format(tmp.ch.dragEff,4)},
     {"color": "#ffffff", "font-size": "20px", "font-family": "Comic Sans MS"}],
     "blank",
     ["display-text",
     function() {if(hasUpgrade('ch',25)) return '你有 ' + format(player.ch.flick) + ' 个红键<br>红键对应效果：Cyten获取量^'+format(tmp.ch.flickEff,4)},
     {"color": "#ffffff", "font-size": "20px", "font-family": "Comic Sans MS"}],
     "blank",
     ["display-text",
     function() {if(hasUpgrade('ch',27)) return '你有 ' + format(player.ch.hold) + ' 个长条<br>长条对应效果：歌曲,源点,Phidata获取指数×'+format(tmp.ch.holdEff,4)},
     {"color": "#ffffff", "font-size": "20px", "font-family": "Comic Sans MS"}],
"buyables",
],
...
buyables:{
 ...
 21: {
    ...
    return cost
				...
				effect(x=player[this.layer].buyables[this.id]) {
				 ...
				 if(clickableEffect('e',13).gt(1)) eff=eff.pow(clickableEffect('e',13))
				 return eff
				},
				display() { // Everything else displayed in the buyable button after the title
     let data = tmp[this.layer].buyables[this.id]
 ...
 22: {
    ...
    return cost.floor()
				...
				effect(x=player[this.layer].buyables[this.id]) {
				 let eff= n(gba('sp',12).mul(x)).pow(0.2).max(1).sub(1).div(10).add(1).pow(tmp.j.pdqja7)
				 if(clickableEffect('e',13).gt(1)) eff=eff.pow(clickableEffect('e',13))
				 return eff
				},
				display() { // Everything else displayed in the buyable button after the title
 ...
 25: {
    ...
    return cost
				...
				effect(x=player[this.layer].buyables[this.id]) {
				 let eff=n(1.01).pow(gba('sp',14)).pow(x).max(1).pow(tmp.j.pdqja7)
				 if(clickableEffect('e',13).gt(1)) eff=eff.pow(clickableEffect('e',13))
				 return eff
				},
				display() { // Everything else displayed in the buyable button after the title
